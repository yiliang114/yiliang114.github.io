...
export function getPagesList(posts) {
  ...
  // 根据标签分组
  for (let item in tagGroup) {
    // 循环标签

    for (let i in pageYearArr) {
      // 循环全部
      const filterTag = pageYearArr[i].pageList.filter(page => {
        // 按标签过滤
    ...
    .map(post => {
      ...
      return {
        ...
        updateTimestamp: new Date(
          post.frontmatter.date || post.lastUpdated
        ).getTime(), // 更新日期的时间戳
        filename: execs ? execs["1"] : "",
        formatDay: formatDate(
          new Date(post.frontmatter.date || post.lastUpdated)
        )
      };
    })
    .sort((a, b) => b.updateTimestamp - a.updateTimestamp)

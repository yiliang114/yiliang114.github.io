...
### [206].反转链表 reverse-linked-list DONE
...
使用三个变量分别表示当前节点和当前节点的前后节点。使用一个变量记录前驱 pre，一个变量记录后继 next. 不断更新`current.next = pre` 就好了
...
树是一种递归的数据结构，因此使用递归方法解决二叉树问题从写法上来看是最简单的。分别在左右子树进行查找 p 和 q。 如果 p 没有在左子树，那么它一定在右子树(题目限定 p 一定在树中)，

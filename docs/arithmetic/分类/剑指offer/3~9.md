...
```js
// 3.1
// 解法 1： 排序后，顺序扫描，判断是否有重复，时间复杂度为 `O(n²)`。
// 解法 2： 时间 O(n) 空间 O(n)
var duplicateInArray = function(nums) {
  ...
  for (const val of nums) {
    if (val > nums.length - 1 || val < 0) return -1;
    if (cached[val]) {
      result.push(val);
    }
    cached[val] = true;
...
// 解法 3： 0 ~ n-1 值与下标会对应起来
var duplicateInArray1 = function(nums) {
...
// console.log(duplicateInArray([2, 3, 5, 4, 3, 2, 6, 7]));
// console.log(duplicateInArray([3, 1, -10, 1, 1, 4, 3, 10, 1, 1]));
// console.log(duplicateInArray([3, 1, 10, 1, 1, 4, 3, 10, 1, 1]));
// console.log(duplicateInArray([]));

console.log(duplicateInArray1([2, 3, 5, 4, 3, 2, 6, 7]));
```

...
弹性盒子布局适合于移动前端开发，它提供一种更加高效的方式来对容器中的条目进行布局、对齐和分配空间
...
这个比较简单，flex 布局给父元素设置 display:flex,左边的子元素 flex:none,给个 width:100px,右边的子元素 flex:1 就可以了，当然可能有别的方式，这个应该是最简单的

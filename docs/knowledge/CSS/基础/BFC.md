...
5. 内部的 Box 会在垂直方向，一个接一个地放置。
6. 属于同一个 BFC 的两个相邻的 Box 的 margin 会发生重叠
7. BFC 就是页面上的一个隔离的独立容器，容器里面的子元素不会影响到外面的元素。反之也如此, 文字环绕效果，设置 float
8. BFC 的区域不会与 float box 重叠。
9. 计算 BFC 的高度，浮动元素也参与计算
...
（Inline formatting contexts）：内联格式上下文

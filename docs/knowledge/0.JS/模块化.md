...
1. 立即执行函数
2. **AMD**：异步模块定义。`requirejs` 在推广过程中对模块定义的规范化产出，提前执行，推崇依赖前置
3. **CMD**：通用模块定义。`seajs` 在推广过程中对模块定义的规范化产出，延迟执行，推崇依赖就近
4. **CommonJs**：主要用在服务器端 node 。输出的是值拷贝。
5. **ES6 Module**：ES6 模块。模块输出的是一个值的引用，编译时输出接口。

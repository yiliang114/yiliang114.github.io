...
3. **组件无法访问声明周期函数。无状态组件是不需要生命周期管理和状态管理，所以底层实现这种形式的组件时不会实现组件的声明周期方法。**
...
在 React 组件的声明周期中，有一个 shouldComponentUpdate 方法，这个方法默认返回值是 true，这就以为这就算没有改变组件的`props` 和 `state` 也会导致组件的重绘。

...
2. `window.onerror`函数。这个函数是全局的。window.onerror 或者 window.addEventListener('error', function(){}, false) 这个只能捕获即时运行错误，资源加载错误因为不会冒泡，所以不会被 window.onerror 捕获到。
...
目前主流的上报方式是利用 img 标签的 src 属性发送请求，例如：
...
关于时间计算: html5 新增的接口 performance
白屏时间计算 ？

...
module.exports = ({
  ...
  return {
    ...
    extendPageData($page) {
      ...
      // 修改 permalink 的前缀。 可以在 md 中自己添加前缀。
      const { prefix = '' } = $page.frontmatter;
      if (prefix) {
        $page.frontmatter.permalink = $page.frontmatter.permalink.replace(/^\/\w+\//, `/${prefix}/`);
        // TODO: 也可以设置成直接从 path 中读取

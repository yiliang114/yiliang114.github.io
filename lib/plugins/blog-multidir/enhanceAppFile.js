// enhanceAppFile 增强 App 的能力。它是在页面上执行的
...
class Classifiable {
  ...
  getItemByKeys(pageKey) {
    for (const dir in this._metaMap) {
      if (dir !== pluginConfig.aliasesRoot && this._metaMap[dir]) {
  ...
  getItemByName(name, current) {
    const { paginationLimit, paginationPath } = pluginConfig
    ...
    const pagination =
      page > 1 ?
        Array.from(Array(page), (v, k) => {
            return k ? path + paginationPath + (k + 1) + '/' : path
          }) :
        []
    ...
    return {
      pagination,
      path,
      pageKeys: current ? this.sliceItem(pageKeys, str, end) : pageKeys,
      posts: current ? this.sliceItem(posts, str, end) : posts
    }
  }
}
